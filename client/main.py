...
class Game(ShowBase):
    ...
    def start(self, username):
        self.connection_manager.wait_for_connection(self.__ready_handler, username)
        self.connection_manager.subscribe_for_new_player(self.__new_player_handler)
    ...
    def __ready_handler(self, game_config: GameConfig):
        ...
        self.connection_manager.subscribe_for_new_player(self.__new_player_handler)
        self.connection_manager.subscribe_for_game_state_change(self.__game_state_change)
    ...
    def attach_input(self):
        self.accept("w", lambda: self.__handle_input("+forward"))
        self.accept("w-up", lambda: self.__handle_input("-forward"))
        self.accept("s", lambda: self.__handle_input("+backward"))
        self.accept("s-up", lambda: self.__handle_input("-backward"))
        self.accept("d", lambda: self.__handle_input("+right"))
        self.accept("d-up", lambda: self.__handle_input("-right"))
        self.accept("a", lambda: self.__handle_input("+left"))
        self.accept("a-up", lambda: self.__handle_input("-left"))
        self.accept("space-up", lambda: self.__handle_bullet())
        self.accept("q", lambda: self.__handle_flag_drop())
